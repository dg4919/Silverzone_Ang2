    ...
    public class BookController : ApiController
    {
        ...
        public IHttpActionResult get_bookDetail_byId(int bookId)
        {
            ...
            var comboDetail = BundleViewModel.Parse(
                              bundleRepository.FindBy(x => bundleId_list.Contains(x.Id) && x.is_Active == true)
                                );
        ...
        public IHttpActionResult getbook_suggestions(int bookId)
        {
            ...
            bookRepository.GetByStatus(true)
                            .Where(x => x.ClassId == bookInfo.ClassId && x.SubjectId == bookInfo.SubjectId && x.Id != bookInfo.Id)
        ...
        public IHttpActionResult getbook_recommends(int bookId)
        {
            ...
            var books = BookViewModel.Parse(
            bookRepository.GetByStatus(true)
                            .Where(x => x.ClassId == bookInfo.ClassId && x.CategoryId == bookInfo.CategoryId && x.Id != bookInfo.Id)
        ...
        public IHttpActionResult getbook_bundles(int classId)
        {
            var bundles = BundleViewModel.Parse(
                              bundleRepository.FindBy(x => x.ClassId == classId && x.is_Active == true)
                                );
        ...
        public IHttpActionResult searchBooks(
            int classId,                                        // We must have a classId while srearciing a record
            [FromUri] ICollection<int> book_categoriesId,      // to support array type use > FromUri
