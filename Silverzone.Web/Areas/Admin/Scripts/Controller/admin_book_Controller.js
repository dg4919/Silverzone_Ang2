...
(function () {
    ...
    var book_createfn = function ($sc, $rsc, svc, sharedSvc, $modal, $stateParams, $state, $timeout) {
        ...
        if ($stateParams.bookId !== undefined) {
            svc.get_bookInfo($stateParams.bookId).then(function (d) {
                ...
                $sc.get_class($sc.bookModel.bookInfo.classId);
            });
        }
    ...
    var bundle_createfn = function ($rsc, svc, $stateParams, $state, $timeout) {
        var that = this;
    ...
    angular.module('Silverzone_app')
        ...
        .controller('bundle_create', ['$rootScope', 'admin_book_Service', '$stateParams', '$state', '$timeout', bundle_createfn])
